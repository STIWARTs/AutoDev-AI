...
export async function handlePullRequest(
  ...
  try {
    ...
    sections.push("## 🗺️ AutoDev Onboarding Context\n");
    ...
    if (conventions.length > 0) {
      ...
      for (const conv of relevantConventions) {
        sections.push(
          `- **${conv.category}** — \`${conv.pattern}\`: ${conv.description}`
        );
      }
      sections.push("");
    }

    const appUrl = process.env.NEXT_PUBLIC_APP_URL || process.env.FRONTEND_URL || "https://autodev.vercel.app";
    sections.push(
      `\n> 💡 [View full architecture & walkthroughs on AutoDev](${appUrl}/dashboard/${repoFullName.replace("/", "%2F")})\n`
    );
    sections.push("*Generated by AutoDev — AI-powered codebase onboarding*");
